let Post,mongoose,Joi,sanitizeHtml;_df0‍.x([["getPostById",()=>getPostById],["write",()=>write],["list",()=>list],["read",()=>read],["remove",()=>remove],["update",()=>update],["checkOwnPost",()=>checkOwnPost]]);_df0‍.w("../../models/post",[["default",["Post"],function(v){Post=v}]]);_df0‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_df0‍.w("joi",[["default",["Joi"],function(v){Joi=v}]]);_df0‍.w("sanitize-html",[["default",["sanitizeHtml"],function(v){sanitizeHtml=v}]]);
...
const sanitizeOption = {
    ...
    try {
        await post.save();
        ctx.body = post;
        Counter.findOne({postCount: 1}, (err, res) =>{
            if (err) return err;
            let up = res.postCountCounter + 1;
        Counter.updateOne({postCount: 1}, {postCountCounter: up}, (err)=> {
            if (err) return err;
        post.updateOne({ postCount: up}, (err) => {
            if(err)  return err;})
        })
    })
    } catch(e) {
